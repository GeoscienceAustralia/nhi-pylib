...
ONEMINUTESTNDTYPE = {'id': str, 'stnNum': int, 'rainfalldist': str,
                     'stnName': str,
                     'stnOpen': str, 'stnClose': str, 'stnLat': float,
                     'stnLon': float, 'stnLoc': str, 'stnState': str,
                     'stnElev': float, 'stnBarometerElev': float,
                     'stnWMOIndex': int, 'stnDataStartYear': str,
                     'stnDataEndYear': str, 'pctComplete': float,
                     'pctY': float, 'pctN': float, 'pctW': float,
                     'pctS': float, 'pctI': float, 'end': str}

ONEMINUTESTNNAMES = list(ONEMINUTESTNDTYPE.keys())

ONEMINUTEDTYPE = {'id': str, 'stnNum': int,
...
DAILYSTNDTYPE = {'id': str, 'stnNum': int, 'rainfalldist': str, 'stnName': str,
                 'stnOpen': str, 'stnClose': str, 'stnLat': float,
                 'stnLon': float, 'stnLoc': str, 'stnState': str,
                 'stnElev': float, 'stnBarmoeterElev': float,
                 'stnWMOIndex': int, 'stnDataStartYear': str,
                 'stnDataEndYear': str, 'pctComplete': float,
                 'pctY': float, 'pctN': float, 'pctW': float,
                 'pctS': float, 'pctI': float, 'end': str}
...
DAILYDTYPE = {'id': str, 'stnNum': int,
              'YYYY': int, 'MM': int, 'DD': int,
              'windgust': float, 'windgustq': str,
              'winddir': float, 'winddirq': str,
              'HHMM': int, 'timeq': str,
              'PresentWx00': int, 'PresentWx00Q': str,
              'PresentWx03': int, 'PresentWx03Q': str,
              'PresentWx06': int, 'PresentWx06Q': str,
              'PresentWx09': int, 'PresentWx09Q': str,
              'PresentWx12': int, 'PresentWx12Q': str,
              'PresentWx15': int, 'PresentWx15Q': str,
              'PresentWx18': int, 'PresentWx18Q': str,
              'PresentWx21': int, 'PresentWx21Q': str,
              'PastWx00': int, 'PastWx00Q': str,
              'PastWx03': int, 'PastWx03Q': str,
              'PastWx06': int, 'PastWx06Q': str,
              'PastWx09': int, 'PastWx09Q': str,
              'PastWx12': int, 'PastWx12Q': str,
              'PastWx15': int, 'PastWx15Q': str,
              'PastWx18': int, 'PastWx18Q': str,
              'PastWx21': int, 'PastWx21Q': str,
              'end': str
              }
